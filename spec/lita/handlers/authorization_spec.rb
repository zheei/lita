...
describe Lita::Handlers::Authorization, lita_handler: true do
  let(:authorization) do
    auth = Lita::Authorization.new(robot)
    allow(auth).to receive(:user_is_admin?).and_return(true)
    auth
  end

  before do
    allow(Lita::Authorization).to receive(:new).and_return(authorization)
  end
    ...
    context "when there are populated groups" do
      ...
      before do
        groups.each do |group|
          authorization.add_user_to_group(user, user1, group)
          authorization.add_user_to_group(user, user2, group)
        end
      end
